...
export default function DealsPipeline() {
  ...
  const filteredDeals = useMemo(() => {
    ...
    return deals.filter((d) => d.title.toLowerCase().includes(q) || (clients.find((c) => c.id === d.client_id)?.name || "").toLowerCase().includes(q));
  }, [deals, clients, query]);

  const itemsByColumn = useMemo(() => {
    const map: Record<string, Deal[]> = {};
    for (const stage of DEAL_STAGES) map[stage] = filteredDeals.filter((d) => d.stage === stage);
    return map;
  }, [filteredDeals]);
      ...
      <div className="flex flex-col h-[calc(100vh-160px)]">
        <div className="space-y-stack-lg flex-shrink-0">
          <PageHeader
            ...
            subtitle={`Drag and drop leads to update their status. ${openDeals} active deals.`}
            right={
        ...
        <div className="flex-1 min-h-0 mt-gutter">
          <KanbanBoard<Deal>
            ...
            renderColumnRight={(_key, items) => (
              <span className="text-xs font-semibold text-on-surface-variant">
                {formatCurrency((items as Deal[]).reduce((s, d) => s + d.value, 0), company.currency)}
              </span>
            )}
            renderCard={(deal) => {
                ...
                <div className="bg-surface-container-lowest rounded-lg border border-outline-variant p-3 shadow-soft hover:shadow-soft-hover transition-shadow">
                  ...
                  {client && (client.whatsapp || client.phone) && (
                    <div className="flex items-center justify-between pt-2 border-t border-outline-variant/60" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button onClick={() => navigate(`/crm/clients/${client.id}`)} className="text-xs font-bold text-primary hover:underline">
                        View
                      </button>
                    </div>
                  )}
